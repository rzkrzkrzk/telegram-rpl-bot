...
async def stick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[1] == "accept":
        owner = int(data[2])
        if user.id == owner: return await query.answer("Это ваш поиск!")
    ...
    if data[1] == "ai":
        owner = int(data[2])
        if user.id != owner: return await query.answer("Только создатель!")
    ...
    if mid not in active_stick_matches: return await query.answer("Матч окончен.")
    m = active_stick_matches[mid]
    
    if action == "s": # Shoot
        if user.id != m["turn"]: return await query.answer("Не ваш ход!")
        m["last_shot"] = choice
        m["phase"] = "defend"
        ...
        defender = m["p2"] if m["turn"] == m["p1"]["id"] else m["p1"]
        if user.id != defender["id"]: return await query.answer("Сейчас не ваша очередь отбивать!")
...
async def process_stick_round(query, context, mid, def_choice):
    ...
    # Гол? Девятки (C) кроют R и L. Домик (D) кроет D.
    scored = True
    if def_choice == "C" and shot in ["R", "L"]: scored = False
    if def_choice == "D" and shot == "D": scored = False
    
    if scored: attacker["goals"] += 1
...
async def ratingmmr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows: return await update.message.reply_text("Рейтинг пуст.")
    txt = "🥇 ТОП-10 MMR Клюшек:\n" + "\n".join([f"{i+1}. {r[0]} - {r[1]}" for i, r in enumerate(rows)])
    await update.message.reply_text(txt)

# ---------- ЛОГИКА ДУЭЛИ БУЛЛИТОВ (ОБНОВЛЕНО 3 GIF) ----------
...
async def admin_callback_games(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_bul_gifs":
        ...
        conn = sqlite3.connect(DB_PATH); conn.execute("DELETE FROM stick_stats"); conn.commit(); conn.close()
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎮 Настройки игры": await show_game_settings(update, context)
    elif text == "🏠 Главное меню": await main_menu(update, context)
    elif text == "🚪 Выйти":
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("^🎮 Настройки игры$|^🏠 Главное меню$|^🚪 Выйти$"), admin_buttons))
